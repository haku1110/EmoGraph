...
import com.kha98.emograph.ui.components.EmotionCard
...
fun TimelineScreen(
    ...
    val pendingExportType = viewModel.pendingExportType
        ...
        topBar = {
            TopAppBar(
                title = {
                    if (isSelectionMode) {
                        Text(
                            text = if (selectedIds.isEmpty()) "エクスポートするデータを選択"
                                   else "${selectedIds.size}件選択中",
                ...
                actions = {
                    if (isSelectionMode) {
                        // 全選択 / 全解除
                        val allSelected = filteredRecords.isNotEmpty() &&
                            selectedIds.containsAll(filteredRecords.map { it.id })
                        IconButton(onClick = {
                            if (allSelected) viewModel.cancelExport()
                            else viewModel.selectAll(filteredRecords)
                        }) {
                            Icon(
                                if (allSelected) Icons.Filled.CheckBox else Icons.Outlined.CheckBox,
                                contentDescription = if (allSelected) "全解除" else "全選択"
                            )
                        }
                        // キャンセル
                        ...
                        // コピーボタン → 選択モードへ
                        IconButton(onClick = { viewModel.startExport(ExportType.COPY) }) {
                            Icon(Icons.Default.ContentCopy, contentDescription = "コピーするデータを選択")
                        }
                        // 共有ボタン → 選択モードへ
                        IconButton(onClick = { viewModel.startExport(ExportType.SHARE) }) {
                            Icon(Icons.Default.IosShare, contentDescription = "共有するデータを選択")
                        }
                        IconButton(onClick = {
            ...
            if (isSelectionMode) {
                BottomAppBar(
                    ...
                    Row(
                        ...
                        Button(
                            onClick = {
                                // confirmExport() が pendingExportType をリセットするため先に取得
                                val exportType = pendingExportType ?: return@Button
                                val markdown = viewModel.confirmExport()
                                when (exportType) {
                                    ExportType.COPY -> {
                                        ...
                                        clipboard.setPrimaryClip(
                                            ClipData.newPlainText("EmoGraph", markdown)
                                        )
                                        Toast.makeText(
                                            context, "クリップボードにコピーしました", Toast.LENGTH_SHORT
                                        ).show()
                                    }
                                    ExportType.SHARE -> {
                                        ...
                                        context.startActivity(
                                            Intent.createChooser(intent, "エクスポート")
                                        )
                                    }
                                }
                            },
                            ...
                            Text(
                                when (pendingExportType) {
                                    ExportType.COPY -> "コピーする"
                                    ExportType.SHARE -> "共有する"
                                    null -> "確定"
                                }
                            )
                        }
                        ...
                        LazyColumn(
                            ...
                            items(filteredRecords, key = { it.id }) { record ->
                                EmotionCard(
                                    ...
                                    onToggleSelect = { viewModel.toggleRecord(record.id) },
...
private fun FilterSheetContent(
    ...
    Column(
        ...
        Row(
            ...
            TextButton(onClick = { viewModel.resetFilters(); onDismiss() }) { Text("リセット") }
